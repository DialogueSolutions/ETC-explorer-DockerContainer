...
nohup pm2 start /.ethereum-classic/process.json &

#Start Geth on Morden chain and fast sync DO this first before you start with mining
nohup geth --sputnikvm --chain=morden --fast --identity=<nameOfGethContainer> --rpc &

#Start mongoDB and start explorer on configred port in the config file.
service mongodb start &&
cd /.ethereum-classic/explorer &&
node app.js

#EXTRA GETH COMMANDS EXAMPLES
